...
class NavClient(object):
	def __init__(self):
		...
		# rospy.Subscriber("gps",NavSatFix,self.gps_cb)												 # This need to be changed
		rospy.Subscriber("/piksi/navsatfix_best_fix",NavSatFix,self.gps_cb)
	...
	def send_goal(self):
		...
		if args.input_file is None and not args.auto:
			info = "input file and auto_generation not defined"
			rospy.loginfo(info)
			sys.exit()
